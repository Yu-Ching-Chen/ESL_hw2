...
void Testbench::do_median_and_mean() {
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
      ...
      if (x == 0) {
        ...
        for (w = -yBound; w != yBound + adjustY; ++w) { // -1, 0, 1
          for (l = -xBound; l != xBound + adjustX; ++l) { // -1, 0, 1
            ...
            if (l == -xBound && w == -yBound) write_data_to_filter(1, R, G, B);
            else write_data_to_filter(2, R, G, B);
          }
        }
          ...
          if (v == -yBound) {
            for (w = -yBound; w != yBound + adjustY; ++w) { // -1, 0, 1
              for (l = -xBound; l != xBound + adjustX; ++l) { // -1, 0, 1
                ...
                if (l == -xBound && w == -yBound) write_data_to_filter(0, R, G, B);
                else write_data_to_filter(2, R, G, B);
              }
            }
          } else {
      ...
      int output_num=0;
      while(!done){
        initiator.read_from_socket(FILTER_MM_BASE + FILTER_CHECK_ADDR, mask, data.uc, 4);
        output_num = data.sint;
        if(output_num>0) done=true;
      }
      wait(10 * CLOCK_PERIOD, SC_NS);

      initiator.read_from_socket(FILTER_MM_BASE + FILTER_RESULT_ADDR, mask,
                                 data.uc, 4);
...
void Testbench::write_data_to_filter(int next_row_control, 
    unsigned char R, unsigned char G, unsigned char B) {
        word data;
        unsigned char mask[4];

        data.uc[0] = next_row_control;
        data.uc[1] = R;
        data.uc[2] = G;
        data.uc[3] = B;
        mask[0] = 0x03;
        ...
        initiator.write_to_socket(FILTER_MM_BASE + FILTER_R_ADDR, mask,
                                  data.uc, 4);
